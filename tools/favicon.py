#!/usr/bin/env python3
"""The site icon: "kb" in Bricolage Grotesque 800 (Outie's heading face), white on cobalt.
...
BG, FG = "#1d3ec4", "#ffffff"   # Outie's --block and its type

def svg(height, rx, track=-30, size=64):
    k, b = G["k"], G["b"]
    ...
    g = lambda d, dx: f'<path transform="translate({tx + dx*s:.2f} {ty:.2f}) scale({s:.5f} {-s:.5f})" d="{d}" fill="{FG}"/>'
    return (f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {size} {size}">'
            f'<rect width="{size}" height="{size}" rx="{rx}" fill="{BG}"/>{g(k["d"], 0)}{g(b["d"], bx)}</svg>')
...
tab, large, apple, mask = svg(34, 14), svg(30, 14), svg(30, 0), svg(24, 0)
...
pngs = []
for px in (16, 32, 48):
    p = os.path.join(tempfile.gettempdir(), f"kb-ico-{px}.png"); png(tab, px, p); pngs.append((px, open(p, "rb").read()))
