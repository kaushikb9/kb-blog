...
test("build emits every route the old site had", () => {
  for (const need of ["/index.html", "/index.xml", "/sitemap.xml", "/404.html", "/_redirects",
                      "/blog/index.html", "/posts/index.html", "/about/index.html", "/ideas/index.html", "/traces/index.html", "/shelf/index.html", "/projects/index.html", "/tags/index.html"])
...
test("projects.md: every row has a name and a line; links are https; images exist", () => {
  ...
  for (const t of projects.talks || []) {
    const who = `content/projects.md talk "${t.title || "?"}"`;
    assert.ok(t.title, `${who}: no title`);
    assert.ok(t.url, `${who}: no url — a talk row exists to link slides or video`);
    if (t.image) assert.ok(fs.existsSync(path.join(ROOT, "content", "projects", t.image)), `${who}: image content/projects/${t.image} does not exist`);
  }
});
